...
pub struct TimeStamp(clock::KrazyKraigTime);

#[cfg(not(target_os = "windows"))]
#[derive(Copy, Clone, PartialEq, Eq, Hash, PartialOrd, Ord)]
pub struct TimeStamp(std::time::SystemTime);

pub type TimeStampError = std::time::SystemTimeError;
...
impl Add<Duration> for TimeStamp {
    ...
    fn add(self, dur: Duration) -> TimeStamp {
        TimeStamp(self.0.checked_add(dur).expect("overflow when adding duration to instant"))
    }
}
...
impl Sub<Duration> for TimeStamp {
    ...
    fn sub(self, dur: Duration) -> TimeStamp {
        TimeStamp(self.0.checked_sub(dur).expect("overflow when subtracting duration from instant"))
    }
}
...
#[cfg(target_os="windows")]
impl TimeStamp {
    pub const UNIX_EPOCH: TimeStamp = TimeStamp(clock::KrazyKraigTime::UNIX_EPOCH);

    pub fn now() -> TimeStamp {
        Self(clock::KrazyKraigTime::now())
    }

    pub fn duration_since(&self, earlier: TimeStamp) -> Result<Duration, TimeStampError> {
        self.0.sub_time(&earlier.0).map_err(TimeStampError)
    }

    pub fn elapsed(&self) -> Result<Duration, TimeStampError> {
...
#[cfg(not(target_os="windows"))]
impl TimeStamp {
    pub const UNIX_EPOCH: TimeStamp = TimeStamp(std::time::SystemTime::UNIX_EPOCH);
